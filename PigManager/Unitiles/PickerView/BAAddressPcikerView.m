...
@interface BAAddressPcikerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@property (nonatomic, copy) BAAddressResultBlock resultBlock;

@end


@implementation BAAddressPcikerView
...
{
    BAAddressPcikerView *addressPickerView = [[BAAddressPcikerView alloc] initWithTitle:title DefaultSelected:defaultSelectedArr FileName:nil IsAutoSelect:isAutoSelect Manager:manager ResultBlock:resultBlock];
...
+ (void)showAddressPickerWithTitle:(NSString *)title
                   DefaultSelected:(NSArray *)defaultSelectedArr
                      ...
                      IsAutoSelect:(BOOL)isAutoSelect
                       ...
                       ResultBlock:(BAAddressResultBlock)resultBlock
{
    BAAddressPcikerView *addressPickerView = [[BAAddressPcikerView alloc] initWithTitle:title DefaultSelected:defaultSelectedArr FileName:fileName IsAutoSelect:isAutoSelect Manager:manager ResultBlock:resultBlock];
    [addressPickerView showWithAnimation:YES];
}
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    BAProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
    BACityModel *cityModel = provinceModel.city[rowOfCity];
    if (component == 0) {
        //返回省个数
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {//省
        BAProvinceModel *provinceModel = self.addressModelArr[row];
        showTitleValue = provinceModel.name;
    }
    if (component == 1) {//市
        BAProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        BACityModel *cityModel = provinceModel.city[row];
        showTitleValue = cityModel.name;
    }
    if (component == 2) {//区
        BAProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        BACityModel *cityModel = provinceModel.city[rowOfCity];
        BATownModel *townModel = cityModel.town[row];
        ...
        BAProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
            if (rowOfCity < provinceModel.city.count) {
                ...
                if (rowOfTown < cityModel.town.count) {
                    BATownModel *townModel = cityModel.town[rowOfTown];
...
- (void)scrollToRow:(NSInteger)firstRow secondRow:(NSInteger)secondRow thirdRow:(NSInteger)thirdRow {
    if (firstRow < self.addressModelArr.count) {
        ...
        BAProvinceModel *provinceModel = self.addressModelArr[firstRow];
        if (self.defaultSelectedArr.count == 2) {
        ...
        if (self.defaultSelectedArr.count == 3) {
            if (secondRow < provinceModel.city.count) {
                rowOfCity = secondRow;
                [self.pickerView reloadComponent:1];
                BACityModel *cityModel = provinceModel.city[secondRow];
